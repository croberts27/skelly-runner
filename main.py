...
class Sprite:
    ...
    def jump(self):
        if self.player_rect.bottom >= 615:  # Check if the player is on the ground
            self.player_gravity = -25  # Apply an initial jump velocity
    ...
    def enemy_attack_animation(self):
        ...
        if self.enemy_attack_index >= len(self.enemy_attack):
            self.enemy_attack_index = 0
            self.is_attacking = False  # Reset back to walking state
...
while running:
    ...
    sprite.player_gravity += 1
    sprite.player_rect.y += sprite.player_gravity  # Update player's position based on gravity
    if sprite.player_rect.bottom >= 615:
        sprite.player_rect.bottom = 615
    sprite.player_animation()
    screen.blit(sprite.player_surf, sprite.player_rect)
    sprite.enemy_animation()
    sprite.enemy_attack_animation()
    screen.blit(sprite.enemy_surf, sprite.enemy_rect)
    screen.blit(sprite.enemy_attack_surf, sprite.enemy_attack_rect)
